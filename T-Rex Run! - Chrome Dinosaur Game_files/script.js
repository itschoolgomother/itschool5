var currentPage=window.location.pathname.split("/")[1];if(document.getElementById("hint")){var hint=document.getElementById("hint");if(currentPage=="t-rex"){if(navigator.userAgent.indexOf('MSIE')!==-1||navigator.appVersion.indexOf('Trident/')>0){hint.style.display="block";}}else{hint.style.display="block";}};var onclickLog=function(){var actionCategory="NavBar"
if(this.className.indexOf("nav-left")>0)
actionCategory="ArrowLeft";else if(this.className.indexOf("nav-right")>0)
actionCategory="ArrowRight";gtag('event',"click",{'event_category':actionCategory,'event_action':'From_'+location.pathname,'event_label':'To_'+this.pathname});};var aObjs=document.getElementsByClassName("nav_str");if(aObjs){for(var i=0;i<aObjs.length;i++){aObjs[i].onclick=onclickLog;}}
